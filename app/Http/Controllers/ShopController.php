...
class ShopController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('admin.shop')->with('success', 'Thêm mới sản phẩm thành công.');
        } catch (\Throwable $th) {
...
'price' => 'required|numeric|min:0',
            ...
            $images_exist = $request->input('images-exist') != null ? json_decode($request->input('images-exist'), true) : [];
            ...
            foreach ($oldImage as $image) {
                if (isset($image) && !in_array($image, $images_exist)) {
                    $deleteImage[] = $image;
            ...
            return redirect()->route('admin.shop')->with('success', 'Cập nhật sản phẩm thành công.');
        } catch (\Throwable $th) {
            return redirect()->back()->withInput()->with('error', $th->getMessage());
        }
    }
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('admin.shop')->with('success', 'Xóa shop thành công.');
    }
}
