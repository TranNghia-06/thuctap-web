...
@section('content')
    <x-ui.breadcrumb :breadcrumbs="[
        ['url' => 'admin.ticket', 'label' => 'Manage Tickets'],
    ...
    <form class="space-y-4 md:space-y-6 mt-8" action="{{ route('admin.ticket.edit', $data->id) }}" method="POST">
        ...
        <x-ui.alert type="warning">
            If the status is already paid, only the notes can be edited.
        </x-ui.alert>
        ...
        <x-form.input-field name="ticket_count" label="Number of Tickets" type="number" :value="old('ticket_count') ?? $data->ticket_count" :readonly="$data->is_paid"
        ...
        <x-form.textarea-field name="details" label="Notes" :value="old('details') ?? $data->details" required
            placeholder="e.g., Booking for Mr. Nguyen Van A - Phone: 0123456789"
            description="e.g., Booking for Mr. Nguyen Van A - Phone: 0123456789" />
