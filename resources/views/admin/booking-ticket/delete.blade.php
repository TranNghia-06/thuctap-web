...
@section('content')
    ...
    <form class="space-y-4 md:space-y-6 mt-8" action="{{ route('admin.ticket.delete', $data->id) }}" method="POST">
        ...
        <x-ui.alert type="warning">
            Please note this action is irreversible. Consider carefully before deleting.
        </x-ui.alert>

        <x-form.input-field readonly name="customer" label="Customer" :value="$data->user->name . ' - ' . $data->user->email" required
            placeholder="Example: Select exhibition" min="0" />
        ...
        <div>
            ...
            <select required id="exhibition_id" name="exhibition_id" disabled
                ...
                <option selected value="" disabled>{{ __('Select exhibition') }}</option>
        ...
        <x-form.input-field readonly name="start_date" label="Start Date" readonly :value="$data->exhibition->formatted_start_date" required
            placeholder="Example: Select exhibition" min="0" />

        <x-form.input-field readonly name="end_date" label="End Date" readonly :value="$data->exhibition->formatted_start_date" required
            placeholder="Example: Select exhibition" min="0" />

        <x-form.input-field name="ticket_count" label="Ticket Quantity" type="number" :value="old('ticket_count') ?? $data->ticket_count" readonly
            placeholder="Example: Enter number of tickets" min="1" />

        <x-form.textarea-field name="details" label="Notes" :value="old('details') ?? $data->details" required
            placeholder="Example: Booking for customer: John Doe - Phone: 0123456789"
            description="Example: Booking for customer: John Doe - Phone: 0123456789" />
