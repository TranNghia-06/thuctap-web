...
@section('content')
    ...
    <form class="space-y-4 md:space-y-6 mt-8" action="{{ route('admin.post.edit', $data->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <x-form.input-field name="title" label="Tiêu đề bài viết" :value="old('title') ?? $data->title" required placeholder="VD: Triển lãm nghệ thuật" />
        ...
        <div>
            ...
            <select required id="status" name="status"
                ...
                <option value="inactive" {{ $status === 'inactive' ? 'selected' : '' }}>{{ __('Không hiển thị') }}</option>
            </select>
            @error('status')
        ...
        <x-form.textarea-field name="content" label="Nội dung bài viết" :value="old('content') ?? $data->content_html" required placeholder="Nhập nội dung bài viết" />
        ...
        <div>
            ...
            <!-- Upload New Image -->
            <div class="mb-4">
                <label for="thumbnail" class="block mb-1 text-sm text-gray-400">{{ __('Tải ảnh mới') }}</label>
            ...
            <div>
                <label class="block mb-1 text-sm text-gray-400">{{ __('Chọn ảnh hiện có') }}</label>
    ...
    <div id="imageModal" class="fixed inset-0 bg-white bg-opacity-50 flex items-center justify-center hidden">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-4xl p-6 max-h-[80vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-medium text-black">{{ __('Chọn ảnh') }}</h3>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                ...
                @empty
                    <p class="text-gray-400 col-span-full text-center">{{ __('Không có ảnh nào') }}</p>
                @endforelse
            </div>
        </div>
    </div>
