...
@section('content')
    ...
    <form class="space-y-4 md:space-y-6 mt-8" action="{{ route('admin.post.restore', $data->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <x-form.input-field name="title" label="Post Title" :value="old('title') ?? $data->title" readonly placeholder="e.g., Art Exhibition" />

        <div>
            <label for="status" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                {{ __('Status') }}
            </label>
            <select disabled id="status" name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                @if (old('status') ?? $data->status)
                    ...
                    <option value="inactive" {{ $status === 'inactive' ? 'selected' : '' }}>{{ __('Hidden') }}</option>
                @else
        ...
        <div>
            ...
            <img id="imgReview" src="{{ asset('storage/' . $data->thumbnail) }}" class="w-16 h-16 rounded-md mt-2" alt="Image Preview">
