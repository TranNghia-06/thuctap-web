...
@php
    // Định nghĩa danh sách các cột trong bảng danh sách người dùng
    $columns = ['Tên', 'Email', 'Vai trò', 'Trạng thái', 'Hành động'];
@endphp

@section('content')
    <!-- Hiển thị breadcrumb giúp điều hướng -->
    <x-ui.breadcrumb :breadcrumbs="[['url' => 'admin.user', 'label' => 'Người dùng']]" />

    <!-- Phần tiêu đề và nút thêm mới người dùng -->
            ...
            @forelse ($users as $user)
                <x-ui.table-row>
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        <!-- Kiểm tra và hiển thị ảnh đại diện của người dùng -->
                        @if ($user->avatar)
                    ...
                    <!-- Các hành động có thể thực hiện với người dùng -->
                    <td class="px-6 py-4 text-nowrap">
                        <a href={{ route('admin.user.edit', $user->id) }}
                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline {{ $user->role == 'admin' ? 'hidden' : '' }}">Chỉnh
                            sửa</a>

                        <!-- Nếu người dùng đang hoạt động, hiển thị nút Khóa, ngược lại hiển thị Mở khóa -->
