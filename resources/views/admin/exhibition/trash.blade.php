...
@section('content')
    ...
    <!-- Start coding here -->
    <x-common.section-action title="Manage Deleted Exhibitions"
        description="List of exhibitions that have been deleted from the system.">
            ...
            @forelse ($data as $item)
                <x-ui.table-row>
                    ...
                    <td class="px-6 py-4 truncate max-w-[200px]">
                        {{ \Carbon\Carbon::parse($item->start_date)->locale('en')->format('l, d M, Y H:i') }}
                    </td>

                    <td class="px-6 py-4 truncate max-w-[200px]">
                        {{ \Carbon\Carbon::parse($item->end_date)->locale('en')->format('l, d M, Y H:i') }}
                    </td>
