...
@section('content')
    <x-ui.breadcrumb :breadcrumbs="[
        ['url' => 'admin.order', 'label' => 'Manage Orders'],
        ['url' => 'admin.order.create', 'label' => 'Create Order'],
    ]" />
    ...
    <form class="space-y-4 md:space-y-6 mt-8" action="{{ route('admin.order.create') }}" method="POST">
        ...
        <x-form.textarea-field name="notes" label="Notes" :value="old('notes') ?? ''" required
            placeholder="E.g.: Order for customer: Nguyen Van A - Phone: 0123456789" />
                ...
                <div class="space-y-4 max-h-[600px] overflow-y-auto">
                    @forelse ($products as $product)
                        <div class="flex space-x-2 shadow rounded-lg p-4 hover:shadow-xl product-item"
                            ...
                            <div>
                                <h1>{{ $product->name }}</h1>
                                <p class="text-sm text-green-700 font-bold underline">Price: {{ $product->formatted_price }} đ</p>
        ...
        <div>
            ...
            <select required id="is_paid" name="is_paid"
                ...
                <option selected value="" disabled>{{ __('Select payment status') }}</option>

                @if (old('is_paid'))
                    <option value="true" selected>{{ __('Paid') }}</option>
                    <option value="false">{{ __('Unpaid') }}</option>
                @else
                    <option value="false" selected>{{ __('Unpaid') }}</option>
