...
@section('content')
    ...
    <form class="space-y-4 md:space-y-6 mt-8" action="{{ route('admin.order.edit', $data->id) }}" method="POST">
        ...
        <x-ui.alert type="warning">
            If the status is paid, only the notes can be edited.
        </x-ui.alert>
        ...
        <x-form.input-field name="customer" label="Total Amount" :value="$data->formatted_total_price" readonly
            placeholder="e.g. Order for customer: Nguyen Van A - Phone: 0123456789" />
                ...
                @forelse  ($data->orderDetails as $productItem)
                    <div class="flex space-x-2 shadow rounded-lg p-4 hover:shadow-xl product-item">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-green-700">Quantity Purchased: {{ $productItem->quantity }}</p>
                            <p class="text-sm text-gray-500">Description: {{ $productItem->collection->description }}</p>
                        </div>
                    </div>
                @empty
                    <div class="text-sm">
                        <p class="text-red-500 mt-2">The product has been deleted previously.</p>
                    </div>
                @endforelse
            </div>
        </div>
